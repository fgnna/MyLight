...
>   >3. [ImHelper.java](#) : 两个IM的包装辅助类，登出登入初始化，父类有做一些消息发送接收等操作
...
>   >3. [SGCall_V2.java](#) : [核心类](#)该类作为音视频通话的统一调用者，包含当前通电话状态查询，通电话发起调用，
        状态的监听及回调处理，在涉及音视频通信的功能都会调用到该类
>   >4. [MinaSocket.java](#) : 这是一个独台与我们服务进行通信的Socket,同[SocketUtil.java](#)封装调用，在[ReceiveMsgService.java](#)中进行初始化，
